...
sp.tmp <- sapply(sp.tmp, function(x){
    ...
    if(length(tmp) == 1){
        ...
        if(any(
            sapply(tmp, function(i) any(is.na(i) | grepl("NA", i))) | tmp[2] == "sp")){
            tmp = 0
        } else {
            tmp[2] <- tolower(tmp[2])
            tmp = paste(c(tmp[1],tmp[2]),collapse = "_")
        }
    }
    tmp
})
...
sp.tmp <- sapply(sp.tmp, function(x){
    ...
    if(length(tmp) == 1){
        ...
        tmp = sapply(tmp, function(i) str_replace_all(i, "[^[:alnum:]]", ""))
        if(any(
            sapply(tmp, function(i) any(is.na(i) | grepl("NA", i))) | 
            tmp[2] == "sp")){
            tmp = 0
        } else {
            tmp = paste(c(tmp[1],tmp[2]),collapse = "_")
...
N_OCC <- pblapply(1:length(splist$species), function(i) {
    ...
    # cat("Processing species", i, "on", length(splist), "\r")
    ...
    tryCatch (
        {
            ...
            # i = which(splist$species == "Anaphalis_margaritacea")
            ...
            if(!all(is.null(taxon_keys))){
                ...
                for (j in 1:length(taxon_keys)){
                    ...
                    status = rbind(status, 
                                   cbind(data.frame(spdata[ , c('species', "taxonomicStatus")], 
                                                    taxon_key = taxon_keys[j]),
                                         taxono))
                    
                }
                
                status <- status[which(status$taxonomicStatus == "ACCEPTED"), ]
                
                # If multiple species returned select the one with the exact same name
                if(nrow(status)>1){
                    if(any(status$species == spgoing)){
                        status <- status[which(status$species == spgoing),]
                    }
                }
                
                ## If multiple species returned select the phylogenetically closest one (same family or same order)
                if(nrow(status)>1){
        ...
        error = function(e){
            tmp = data.frame(species = splist$species[i])
            write.csv(tmp, paste0("errors/",splist$species[i],".csv"))
...
for(i in 1:length(reqs)){ 
    ...
    if(k <= 3){ # GBIF allow 3 requests at a time
        inforequest[[k]] <- occ_download(pred("taxonKey", 5201569), 
                                         # pred_in("taxonKey", reqs[[1]][i]), 
...
write.csv(requests, paste0("Data/requests.csv"))
...
# Load the very large file downloaded in Option 1 and split into smaller chunks
```{r}

test <- fread("D:/Bioshifts_GBIF/0262389-210914110416597.csv", nrows = 2)
