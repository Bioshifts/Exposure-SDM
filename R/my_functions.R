...
# years = vector of year from each occurrence in sp_occ
# temporal_range = range of years used to generate pseudo-dates
...
# n_pa = number of pseudo-absences. If NULL, n_pa = n occurrences
# temporal_sample = method to sample pseudo-dates. Options area 'range' or 'proportional'. If 'range', 'temporal_range' must be provided, and samples pseudo-dates from the defined temporal_range. If 'proportional', 'years' must be provided, and samples dates proportional to the number of occurrences per year.
create_temporal_pseudo_absences <- function(BA, 
                                            years = NULL, 
                                            temporal_range = NULL,
                                            sp_occ, 
                                            n_pa = NULL,
                                            temporal_sample = 'range'){
    
    if (temporal_sample == "range" && is.null(temporal_range)) {
        stop("temporal_sample = 'range', 'temporal_range' must be provided")
    }
    if(temporal_sample == "proportional" && is.null(years)){
        stop("temporal_sample = 'proportional', 'years' must be provided")
    }
    ...
    # 2) get dates
    if(temporal_sample == "range"){
        temporal_range[1] <- temporal_range[1] + n_yr_bioclimatic # + n_yr_bioclimatic to be able to calculate bioclimatics for the previous n_yr_bioclimatic
        all_dates <- format(
            seq.Date(from = as.Date(paste0(temporal_range[1],"/01/01")), 
                     to = as.Date(paste0(temporal_range[2],"/12/01")), 
                     by = "month"), 
            "%m_%Y")
        # 3) sample random cells and dates
        random_cells <- sample(cells_BA, size = n_pa, replace = TRUE)
        random_dates <- sample(all_dates, size = n_pa, replace = TRUE)
        ...
        while(any(PA_cell_dates %in% occ_cell_dates)){
            ...
            if(run > 100){
                ...
                PA_cell_dates <- sample(all_possible_cells_dates, size = n_pa, replace = TRUE)
    ...
    if(temporal_sample == "proportional"){
        
        # 2) get years proportional to occurrence frequency
        year_freq <- table(years)
        prob_years <- year_freq / sum(year_freq)
        
        # build all possible month–year combinations, weighted by year frequency
        all_dates <- unlist(lapply(names(prob_years), function(y){
            paste(sprintf("%02d", 1:12), y, sep = "_")
        }))
        date_probs <- rep(prob_years, each = 12)
        date_probs <- date_probs / sum(date_probs)
        
        # 3) sample random cells and dates
        random_cells <- sample(cells_BA, size = n_pa, replace = TRUE)
        random_dates <- sample(all_dates, size = n_pa, replace = TRUE, prob = date_probs)
        PA_cell_dates <- paste(random_cells, random_dates)
        
        # 4) remove combinations that coincide with occurrences
        occ_cell_dates <- paste(sp_occ$cell, paste(sp_occ$month, sp_occ$year, sep = "_"))
        run <- 0
        ...
        while (any(PA_cell_dates %in% occ_cell_dates)) {
            ...
            if (run > 100) {
                all_possible_cells_dates <- expand.grid(cells_BA, all_dates)
                all_possible_cells_dates <- paste(all_possible_cells_dates[,1],
                                                  all_possible_cells_dates[,2])
                all_possible_cells_dates <- all_possible_cells_dates[
                    !all_possible_cells_dates %in% occ_cell_dates
                ]
                
                PA_cell_dates <- sample(all_possible_cells_dates, size = n_pa, replace = TRUE)
                tmp <- strsplit(PA_cell_dates, " ")
                random_cells <- as.numeric(sapply(tmp, `[`, 1))
                random_dates <- sapply(tmp, `[`, 2)
                break
            }
        }
    }
...
bioclimatics_from_date <- function(all_layers,
                                   date_i,
                                   realm,
                                   sp_occ,
                                   n_yr_bioclimatic,
...
partitions <- c("normal-amd","normal","bigmem-amd","bigmem")
limits <- data.frame(partition = partitions,
...
check_if_has_sdms_for_all_shifts <- function(all_sps,bioshifts,sdm_dir = sdm_dir){
