# based on bioticVelocity from the R package enmSdmX

# The difference between this metric (bioshifts) and the one on enmSdmX:
# 1) bioshifts identifies if the hemisphere is N or S, and calculates velocities always in the S>N directions if in the N hemisphere, and N>N direction if in the south hemisphere. So our velocity metrics represents velocity away from the tropics.
# 2) quantile shifts are based on latitude, rather then SDM as in the original package. Therefore, quantiles represent shifts in the across different latitudinal positions, defined based on quantiles of the latitude.
...
bioshifts <- function(
    ...
    if (cores > 1L) {
        ...
        export <- c('bioticVelocity', 'bioshifts_euclid', 'bioshifts_cardinalDistance', 'bioshifts_interpCoordFromQuantile', 'nicheOverlapMetrics')
        ...
        for (indexFrom in indicesFrom) {
            ...
            if ('centroid' %in% metrics) {
                ...
                metric <- bioshifts_euclid(c(x1centroidLong, x1centroidLat, x1centroidElev), c(x2centroidLong, x2centroidLat, x2centroidElev))
                metricRate <- metric / timeSpan
                
                # change sign if shifts occur in the south hemisphere >> ensures shifts are positive if towards the poles and negative if towards the tropics
                if(x1centroidLat < 0 & x2centroidLat < 0){
                    metricRate <- metricRate * -1
                }
            ...
            if ('nCentroid' %in% metrics) {
                ...
                cardOut <- bioshifts_cardinalDistance(
                    direction='n',
                    longOrLat=latitude,
                    coordVect=latVect,
                    x1=x1,
            ...
            if ('nsCentroid' %in% metrics) {
                ...
                metric <- bioshifts_euclid(c(x2centroidLat, x2centroidElev), c(x1centroidLat, x1centroidElev))
                ...
                thisOut <- cbind(
                    ...
                    data.frame(
                        ...
                        nsCentroidLat1 = x1centroidLat,
                        nsCentroidLat2 = x2centroidLat
                    ),
                    row.names=NULL
                )
            ...
            if ('ewCentroid' %in% metrics) {
                ...
                metric <- bioshifts_euclid(c(x2centroidLong, x2centroidElev), c(x1centroidLong, x1centroidElev))
                metricRate <- metricSign * metric / timeSpan
            ...
            if ('nsQuants' %in% metrics) {
                ...
                x1loc <- bioshifts_interpCoordFromQuantile(latOrLong='latitude', quants=quants, x=x1, coordVect=latVect, weightedElev=x1weightedElev, warn=warn)
                x2loc <- bioshifts_interpCoordFromQuantile(latOrLong='latitude', quants=quants, x=x2, coordVect=latVect, weightedElev=x2weightedElev, warn=warn)
                ...
                for (countQuant in seq_along(quants)) {
                    ...
                    metric <- bioshifts_euclid(c(x1loc$coord[countQuant], x1loc$elev[countQuant]), c(x2loc$coord[countQuant], x2loc$elev[countQuant]))
                    metric <- metricSign * metric
                    metricRate <- metric / timeSpan
                    
                    # change sign if shifts occur in the south hemisphere >> ensures shifts are positive if towards the poles and negative if towards the tropics
                    if(x1loc$coord[countQuant] < 0 & x2loc$coord[countQuant] < 0){
                        metricRate <- metricRate * -1
                    }
                    ...
                    thisQuantOut <- data.frame(
                        ...
                        nsQuantLat1 = x1loc$coord[countQuant],
                        nsQuantLat2 = x2loc$coord[countQuant]
                    )
            ...
            if ('ewQuants' %in% metrics) {
                ...
                # latitude of all quantiles
                x1loc <- bioshifts_interpCoordFromQuantile(latOrLong='longitude', quants=quants, x=x1, coordVect=longVect, weightedElev=x1weightedElev, warn=warn)
                x2loc <- bioshifts_interpCoordFromQuantile(latOrLong='longitude', quants=quants, x=x2, coordVect=longVect, weightedElev=x2weightedElev, warn=warn)
                ...
                for (countQuant in seq_along(quants)) {
                    ...
                    thisQuantOut <- data.frame(
                        ...
                        ewQuantLong1 = x1loc$coord[countQuant],
                        ewQuantLong2 = x2loc$coord[countQuant]
                    )
            ...
            if ('elevQuants' %in% metrics) {
                ...
                for (countQuant in seq_along(quants)) {
                    ...
                    thisQuantOut <- data.frame(
                        ...
                        elevQuantElevFrom = elevFrom,
                        elevQuantElevTo = elevTo
                    )
...
#' @keywords internal
bioshifts_euclid <- compiler::cmpfun(function(a, b) {
...
#' @keywords internal
bioshifts_cardinalDistance <- function(
        direction,
        longOrLat,
        coordVect,
        x1,
    ...
    if (abundance == 0) {
        ...
        distance <- bioshifts_euclid(c(x2centroidLongCensored, x2centroidLatCensored, x1elevCensored), c(x1centroidLongCensored, x1centroidLatCensored, x2elevCensored))
...
#' @keywords internal
bioshifts_interpCoordFromQuantile <- compiler::cmpfun(
