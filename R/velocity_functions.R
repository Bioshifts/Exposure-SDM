...
temp_grad <- function(x, th, ncores=NULL, file_name="",overwrite=TRUE) {
  if(is.null(ncores)){
...
#--------------
# spatial gradient for big rasters
spatial_grad_big <- function(rx, 
                             y_diff = 1, 
                             unit_out = "km", 
                             n_tiles_col = round(nrow(rx)/100,0), 
  ...
  nrow(rx_tiles) <- n_tiles_nrow
  ncol(rx_tiles) <- n_tiles_col
  # rx_tiles[] <- 1:ncell(rx_tiles)
  # plot(rx_tiles);dev.off()
  rx_tiles <- terra::makeTiles(
    rx, 
    rx_tiles, 
    filename = here(tmp_dir,filename_tiles),
    na.rm = TRUE, overwrite=TRUE)
  
  # x=rx_tiles[1]
  test <- parallel::mclapply(rx_tiles, function(x) {
  ...
  if(length(remove_these)>0){
    tiles_good <- rx_tiles[-remove_these]
  } else {
    tiles_good <- rx_tiles
  }
  ...
  unlink(rx_tiles)
  unlink(gsub("tile","spgrad",rx_tiles))
