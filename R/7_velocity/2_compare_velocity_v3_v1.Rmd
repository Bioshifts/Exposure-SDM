...
# zoom to mediterranean
plot(project(crop(gVel,c(-10,40,30,71)),Eckt), col = the_colors)

# zoom to australia
plot(project(crop(gVel,c(90,180,-30,30)),Eckt), col = the_colors)

```


The velocity pattern identified here, using our function based on the VoCC package, is very similar to what has been presented in a previous study (see Fig 2c from https://www.nature.com/articles/s41558-020-0889-7).
...
Likewise, the temporal trend calculate using our function is very similar to what has been presented in a previous study (see Fig 2a from https://www.nature.com/articles/s41558-020-0889-7).
...
# Fix gVelLat sign in the South cells
gVelLat2 <- gVelLat
if(length(SouthCells)>0){
  gVelLat2[SouthCells] <- gVelLat2[SouthCells] * -1
}

plot(gVelAngle, gVelLat2,
     xlab="Gradient direction", 
     ylab = "Temperature change North")

gVelLat3 <- gVelLat2
gVelLat3[SouthCells] <- NA
gVelAngle3 <- gVelAngle2
gVelAngle3[SouthCells] <- NA

plot(gVelAngle3, gVelLat3,
     xlab="Gradient direction", 
     ylab = "Temperature change North")
...
gVelAngle_coarse <- terra::aggregate(gVelAngle, fact = 5)

## get the set of starting cells for the trajectories
lonlat <- na.omit(as.data.frame(gVelAngle_coarse, xy=TRUE))[,1:2]

## calculate trajectory:
traj = voccTraj(lonlat, # starting lat lon coords
                raster::aggregate(raster::raster(gVel),5), # velocity 
                raster::aggregate(raster::raster(gVelAngle),5), # angle
                raster::aggregate(raster::raster(here::here("Data/Mar_mean_avg_climate_layers.tif")),5), # mean temp 
...
# zoom to Australia for contrast with 10.1111/2041-210X.13295
{
  layout(matrix(c(1,1,1,2), nrow = 1, ncol = 4, byrow=T))
  plot(rast(crop(raster::raster((gVelAngle + 180) %% 360), c(90,180,-30,30))),
       col = myramp)
  plot(globe_lines,add=TRUE)
  plot(lns, add = TRUE, arrows = 1)
  plotrix::polar.plot(
    start = 90,
    lengths = c(rnorm(360,mean = 1, sd = 0.001)),
    polar.pos = seq(0,360,by=1),
...
# Zoom to the Mediterranean for contrast with 10.1126/science.1210288
{
  layout(matrix(c(1,1,1,2), nrow = 1, ncol = 4, byrow=T))
  plot(rast(crop(raster::raster((gVelAngle + 180) %% 360), c(-10,40,30,71))),
       col = myramp)
  # plot(lns, add = TRUE, arrows = 1)
  plotrix::polar.plot(
    start = 90,
