...
range_shift <- function(sui_t1, sui_t2, var_t, probs = c(0, 0.01, 0.05, 0.25, 0.5, 0.75, 0.95, 0.99, 1)){
    
    # calc quantile weighted type for SA in t1
    wti1 <- wtd.quantile(var_t, sui_t1, probs = probs) 
    names(wti1) <- gsub(" ","",names(wti1))
    wti1 <- as.data.frame(wti1)
    wti1$quant <- rownames(wti1)
    # calc mean type for t1
    tmp <- data.frame(as.numeric(wtd.mean(var_t, sui_t1)), "mean")
    names(tmp) <- names(wti1)
    wti1 <- rbind(wti1, tmp)
    rownames(wti1) <- NULL
    
    # calc quantile weighted type for SA in t2
    wti2 <- wtd.quantile(var_t, sui_t2, probs = probs) 
