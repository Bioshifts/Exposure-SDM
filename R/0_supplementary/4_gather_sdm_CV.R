...
occ_mar <- pbapply::pblapply(sdms_mar, function(x){
    file2go <- here::here(sdm_dir("Mar"),x,paste0(x,"_SDM_info.csv"))
    if(file.exists(file2go)){
        tmp <- read.csv(file2go)
    }
})
rem <- sapply(occ_mar, class)
rem <- which(rem=="try-error")
if(any(rem)){
    occ_mar <- occ_mar[-rem]
}
occ_mar <- data.table::rbindlist(occ_mar)
occ_mar$ECO = "Mar"


occ_ter <- pbapply::pblapply(sdms_ter, function(x){
    file2go <- here::here(sdm_dir("Ter"),x,paste0(x,"_SDM_info.csv"))
    if(file.exists(file2go)){
        tmp <- read.csv(file2go)
    }
})
rem <- sapply(occ_ter, class)
rem <- which(rem=="try-error")
if(any(rem)){
    occ_ter <- occ_ter[-rem]
}
occ_ter <- data.table::rbindlist(occ_ter)
occ_ter$ECO = "Ter"

## 2) group all
sdms_occ <- rbind(occ_mar, occ_ter)

## 3) save
write.csv(sdms_occ, here::here(work_dir,"Data","sdms_occ_supp.csv"), row.names = FALSE)
...
## 3) save
write.csv(sdms_CV, here::here("Data","sdms_CV_supp.csv"), row.names = FALSE)
...
## 3) save
write.csv(sdms_Boyce, here::here(getwd(),"Data","sdms_boyce_supp.csv"), row.names = FALSE)
