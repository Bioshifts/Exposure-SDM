...
# output dir
output_dir <- here::here(sdm_dir_supp(realm), sptogo)
...
PresAbs <- qs::qread(here::here(env_data_dir(realm),
                                "Supp",
                                paste0(sptogo,"_",realm,".qs")))
...
write.csv(bioshifts, 
          here::here(output_dir,
                     paste(sptogo,"bioshift.csv")),row.names = FALSE)
