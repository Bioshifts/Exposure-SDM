...
Rscript_file = here::here(script.dir,"4_get_shifts_edge_method.R")
...
# Make sure to change in 4_get_shifts_edges_method.R
# If calculating range shifts with new SDM workflow (this uses all cells in rasters)
sdm_dir <- sdm_dir_supp
shift_dir <- shift_dir_supp

# If calculating range shifts (using all cells in raster)
# sdm_dir <- sdm_dir_supp # use supp SDMs just for listing species of interest
# shift_dir <- shift_dir_supp_recalc # use recalc shifts
...
I_have_shift_ter <- list.files(shift_dir("Ter"), pattern = "SA.csv")
...
I_have_shift_mar <- list.files(shift_dir("Mar"), pattern = "SA.csv")
        ...
        if(!shifttogo %in% test){
            ...
            if(realmtogo == "Ter"){ # for the Terrestrial use this (bigger jobs) 
                ...
                partition = select_partition(
                    ...
                    request_cpu = cores, 
                    limits=limits %>%
                        filter(partition %in% c("normal-amd","normal")))
            }
