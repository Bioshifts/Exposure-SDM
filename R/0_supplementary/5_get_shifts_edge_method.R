...
# sptogo="Acrocarpia_paniculata"
# realm <- "Mar"
# IDtogo = "A112_P1"

# sptogo="Acer_saccharum"
# realm <- "Ter"
# IDtogo = "A85_P1"
...
# sdm_dir <- sdm_dir # unchanged >> uses original SDMs
# shift_dir <- shift_dir_supp_recalc # use recalc shifts

# create dirs
dir.create(sdm_dir(realm), recursive = TRUE, showWarnings = FALSE)
dir.create(shift_dir(realm), recursive = TRUE, showWarnings = FALSE)
...
for(i in 1:nrow(shift_info)){
    ...
    range_shifts <- range_shift(x = sdms_i_togo,
                                SA = NULL,
                                periods = periods_i,
                                raster_size_tolerance = NULL, # 10^5,
                                proj_equal = TRUE)
