...
list.of.packages <- c("raster","rgdal","rgeos","terra","stars",
                      "tidyverse","tictoc","qs","GeoThinneR",
...
# sptogo <- "Spondyliosoma_cantharus"
# realm = "Mar"
...
env_data_dir <- here(env_data_dir(realm),"Supp")

########################
# Check if dir exists for saving files exists
dir.create(env_data_dir, recursive = TRUE, showWarnings = FALSE)
...
########################
if(realm=="Ter"){
    # Spatial thinning of occurrences (use 5km)
    sp_occ_thin <- GeoThinneR::grid_thinning(sp_occ[,c("x","y")], 
                                             thin_dist = 5)
    sp_occ_thin <- which(sp_occ_thin[[1]])
    sp_occ <- sp_occ[sp_occ_thin,]
    nrow(sp_occ)
}
...
# PA_occ <- create_temporal_pseudo_absences(BA, 
#                                           temporal_range = env_range, 
#                                           sp_occ = sp_occ)

n_pa <- ifelse(nrow(sp_occ) <= 1000, 1000, 10000)

PA_occ <- create_temporal_pseudo_absences(BA, 
                                          years = sp_occ$year, 
                                          sp_occ = sp_occ,
                                          n_pa = n_pa,
                                          temporal_sample = "proportional")
...
for(j in 1:length(chunks)){
    ...
    while(trying == 0 | any(sapply(tmp,class)=="try-error")){
        ...
        tmp <- parallel::mclapply(1:length(possibledates_j), function(i) {
            ...
            bios_i <- bioclimatics_from_date(all_layers,
                                             date_i,
                                             realm,
                                             sp_occ,
                                             n_yr_bioclimatic)
...
sp_occ2 <- qread(here::here(env_data_dir,paste0(sptogo,"_",realm,".qs")))
...
summary(sp_occ[which(pa==0)]$min)
summary(sp_occ2[which(pa==0)]$min)
summary(sp_occ3[which(pa==0)]$min)
