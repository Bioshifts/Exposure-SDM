...
list.of.packages <- c("raster","spdep","rgdal","rworldmap","maptools","terra","ff","sf",
...
for(i in 1:length(vars)){
    ...
    myfiles <- here::here(nc_path, vars[i],"_",periods,"_V.2.1.tif")
    # N downloaded files
...
#######
## There are missing variables for precipitation from CHELSA at these dates: 
...
if(any(sapply(my.test, length)>0)){
    ...
    for (v in 1:length(togovars)){
        ...
        pbsapply(varstogo, function(i) {
            ...
            getChelsa(togovars[v], i, here::here(nc_path,"raw"), over.write = FALSE)
...
for(i in 1:length(vars)){ cat(vars[i],"\n")
    ...
    serverfilesize. <- pbsapply(myfilesize[[vars[i]]]$file, function(x) DownloadSizeChelsa(vars[i], x))
...
tmp <- is.na(tmp)
tmp <- ifel(tmp,0,1)
names(tmp) <- "model_raster_ter"

writeRaster(tmp, here::here("/media/seagate/boliveira/Land/model_raster_ter.tif"), overwrite = TRUE)

```

# Make tiles
Divide the world in 40 pieces
```{r}
...
tmp <- rast(list.files(here::here(nc_path,vars[1]),full.names = TRUE)[1])
...
# create tiles
tiles <- rast(nrow = 4, ncol = 10, myext)
...
myfiles <- sapply(vars, function(i){
    if(!dir.exists(here::here(nc_path,i,"Tiles"))){
        dir.create(here::here(nc_path,i,"Tiles"))
    }
    tmp <- list.files(here::here(nc_path,i),full.names = TRUE,pattern = ".tif")
    mclapply(tmp, function(x){
        tmp1 <- rast(x)
        tmp_name <- names(tmp1)
        makeTiles(tmp1, tiles, here::here(nc_path,i,"Tiles",paste0(tmp_name,"_tile.tif")),overwrite=TRUE)
    }, mc.cores = 40)
})
